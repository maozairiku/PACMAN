...
void LoadModel(char* FileName, DX11_MODEL* Model)
{
	...
	Model->VertexArray = model.VertexArray;
	Model->IndexArray = model.IndexArray;
...
void UnloadModel(DX11_MODEL* Model)
{
	...
	if (Model->VertexArray)			delete[] Model->VertexArray;
	if (Model->IndexArray)			delete[] Model->IndexArray;
...
{
	float rayLengthsqr = powf(rayLength, 2);
	for (unsigned short i = 0; i < Model->SubsetNum; i++)
	{
		for (unsigned int j = 0; j < Model->SubsetArray[i].IndexNum; j += 3)
		{
			// 頂点インデックス取得
			unsigned short index0 = Model->IndexArray[Model->SubsetArray[i].StartIndex + j + 0];
			unsigned short index1 = Model->IndexArray[Model->SubsetArray[i].StartIndex + j + 1];
			unsigned short index2 = Model->IndexArray[Model->SubsetArray[i].StartIndex + j + 2];

			// 頂点座標取得
			XMFLOAT3 v0 = Model->VertexArray[index0].Position;
			XMFLOAT3 v1 = Model->VertexArray[index1].Position;
			XMFLOAT3 v2 = Model->VertexArray[index2].Position;

			XMFLOAT3 v0w, v1w, v2w;

			// ワールド座標に変換
			XMMATRIX mtx = XMLoadFloat4x4(&mtxWorld);
			XMStoreFloat3(&v0w, XMVector3TransformCoord(XMLoadFloat3(&v0), mtx));
			XMStoreFloat3(&v1w, XMVector3TransformCoord(XMLoadFloat3(&v1), mtx));
			XMStoreFloat3(&v2w, XMVector3TransformCoord(XMLoadFloat3(&v2), mtx));

			float lengthToV0 = powf(v0w.x - rayPos.x, 2) + powf(v0w.y - rayPos.y, 2) + powf(v0w.z - rayPos.z, 2);
			float lengthToV1 = powf(v1w.x - rayPos.x, 2) + powf(v1w.y - rayPos.y, 2) + powf(v1w.z - rayPos.z, 2);
			float lengthToV2 = powf(v2w.x - rayPos.x, 2) + powf(v2w.y - rayPos.y, 2) + powf(v2w.z - rayPos.z, 2);
			...
			// レイと三角形の当たり判定
			XMFLOAT3 rayEnd = XMFLOAT3(rayPos.x + rayDirection.x * rayLength, rayPos.y + rayDirection.y * rayLength, rayPos.z + rayDirection.z * rayLength);
			if (RayCast(v0w, v1w, v2w, rayPos, rayEnd, hitPos, normal))
			{
				return true;
			}
		}
